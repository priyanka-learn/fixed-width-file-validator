...
---
How It Works
Schema Loading — Reads the field definition Excel and computes cumulative start/end slice positions directly in columns F and G using pure Python.
Record Length Check — Validates that every row in the text file matches the expected total width.
Field-Level Validation — Iterates through each record and each column, slicing the fixed-width string and applying type-specific validation rules.
TRXN Header Validation — If the filename contains `TRXN`, validates the header row structure separately.
Result Classification — Writes validation logs and moves error files to a separate subfolder.
---
